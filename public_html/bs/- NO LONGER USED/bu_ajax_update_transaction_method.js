...
$(document).ready(function() {
    ...
    $("div#update-transaction-method-modal").on("show.bs.modal", function (e) {
        ...
        */ 
        ...
        var DTRowIndex = $('#transaction-methods').DataTable().row($(e.relatedTarget).parents('tr')).index()   // Based on the total number of rows in a table. With a table containing 5,000 records the row index is between 0 and 4,999.
        $('form#update-transaction-method input[type=text][name=dt-row-index]').val(DTRowIndex)
        ...
        $('form#update-transaction-method input[type=text][name=dom-row-index]').val(DOMRowIndex)
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Modal Header
                $(e.currentTarget).find('#staticBackdropLabel').html('View | Update Transaction Method [Record ID <span class="text-grey">' + recordID + '</span>]')

            // Record ID [Hidden]
                $(e.currentTarget).find('form#update-transaction-method input[type=text][name=record-id]').val(data['id'])
            // Entity ID [Read-only]
                $(e.currentTarget).find('form#update-transaction-method #method-id').val(data['method_id'])
            // Entity Description
                $(e.currentTarget).find('form#update-transaction-method #method-description').val(data['method_description'])
            // Used By [Info]
                $(e.currentTarget).find('form#update-transaction-method span#used-by').text('Used by ' + usedBy + ' ' + recordType + ' records')  
            } else {
            // Update failed
                Swal.fire({
                    title: "Failure",
    ...
    $('form#update-transaction-method').on('submit', function(event) {                    
