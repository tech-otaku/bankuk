    ...
    $("div#update-transaction-type-modal").on("show.bs.modal", function (e) {
        ...
        */ 
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Entity ID [Read-only]
                $(e.currentTarget).find('form#update-transaction-type #type-id').val(data['type_id'])
            // Entity Description
                $(e.currentTarget).find('form#update-transaction-type #type-description').val(data['type_description'])
            // Used By [Info]
                $(e.currentTarget).find('form#update-transaction-type span#used-by').text('Used by ' + usedBy + ' ' + recordType + ' records')  
            } else {
            // Update failed
                Swal.fire({
                    title: "Failure",
