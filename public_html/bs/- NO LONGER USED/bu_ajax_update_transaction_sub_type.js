...
$(document).ready(function() {
    ...
    $("div#update-transaction-sub-type-modal").on("show.bs.modal", function (e) {
        ...
        */ 
        ...
        var DTRowIndex = $('#transaction-sub-types').DataTable().row($(e.relatedTarget).parents('tr')).index()   // Based on the total number of rows in a table. With a table containing 5,000 records the row index is between 0 and 4,999.
        console.log('bu_ajax_update_transaction_sub_type.js: DTRowIndex:' + DTRowIndex)
        $('form#update-transaction-sub-type input[type=text][name=dt-row-index]').val(DTRowIndex)
        ...
        $('form#update-transaction-sub-type input[type=text][name=dom-row-index]').val(DOMRowIndex)
               
        var recordID = $(e.relatedTarget).data("record-id")
        var mysqlTable = $(e.relatedTarget).data("mysql-table")
        var usedBy = $(e.relatedTarget).data("sub-type-used-by")
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Modal Header
                $(e.currentTarget).find('#staticBackdropLabel').html('View | Update Transaction Sub-Type [Record ID <span class="text-grey">' + recordID + '</span>]')

            // Record ID [Hidden]
                $(e.currentTarget).find('form#update-transaction-sub-type input[type=text][name=record-id]').val(data['id'])
            // Entity ID [Read-only]
                $(e.currentTarget).find('form#update-transaction-sub-type #sub-type-id').val(data['sub_type_id'])
            // Entity Description
                $(e.currentTarget).find('form#update-transaction-sub-type #sub-type-description').val(data['sub_type_description'])
            // Used By [Info]
                $(e.currentTarget).find('form#update-transaction-sub-type span#used-by').text('Used by ' + usedBy + ' ' + recordType + ' records')  
            } else {
            // Update failed
                Swal.fire({
                    title: "Failure",
    ...
    $('form#update-transaction-sub-type').on('submit', function(event) {                    
