        ...
        */ 
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Account ID [Read-only]
                $(e.currentTarget).find('form#update-regular-debit #account-id-alpha-read-only').val(data['account_id_alpha'])
            ...
            // Type [Dropdown]
                $(e.currentTarget).find('form#update-regular-debit #type-id').val(data['type_id']).change()
            // Sub-type [Dropdown]
                $(e.currentTarget).find('form#update-regular-debit #sub-type-id').val(data['sub_type_id']).change()
            // Entity [Dropdown]
                $(e.currentTarget).find('form#update-regular-debit #entity-id').val(data['entity_id']).change()
            // Entity [Dropdown]
                $(e.currentTarget).find('form#update-regular-debit #method-id').val(data['method_id']).change()
            // Day
