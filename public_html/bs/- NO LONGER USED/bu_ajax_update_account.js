...
$(document).ready(function() {
    ...
    $("div#update-account-modal").on("show.bs.modal", function (e) {
        ...
        */ 
        ...
        var DTRowIndex = $('#accounts').DataTable().row($(e.relatedTarget).parents('tr')).index()   // Based on the total number of rows in a table. With a table containing 5,000 records the row index is between 0 and 4,999.
        $('form#update-account input[type=text][name=dt-row-index]').val(DTRowIndex)
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Modal Header
                $(e.currentTarget).find('#staticBackdropLabel').html('View | Update Account [Record ID <span class="text-grey">' + recordID + '</span>]')

            // Record ID [Hidden]
                $(e.currentTarget).find('form#update-account input[type=text][name=record-id]').val(data['id'])
            // Account ID Alpha [Read-only]
                $(e.currentTarget).find('form#update-account #account-id-alpha-read-only').val(data['account_id_alpha'])
            // Account ID [Read-only]
                $(e.currentTarget).find('form#update-account #account-id').val(data['account_id'])
            // Bank's Trading Name [Dropdown]
                $(e.currentTarget).find('form#update-account #bank-id').val(data['bank_id']).change()
            // Account Name
                $(e.currentTarget).find('form#update-account #account-name').val(data['name'])
            // Sort Code
                $(e.currentTarget).find('form#update-account #sort-code').val(data['sort_code'])
            // Account Number
                $(e.currentTarget).find('form#update-account #account-number').val(data['account_number'])
            // Account Status [Dropdown]
                $(e.currentTarget).find('form#update-account #status').val(data['status']).change()
            // Used By [Info]
                $(e.currentTarget).find('form#update-account span#used-by').text('Used by ' + usedBy + ' ' + recordType + ' records')
            // Notes
                $(e.currentTarget).find('form#update-account #notes').val(data['notes'])
            } else {
            // Update failed
                Swal.fire({
                    title: "Failure",
    ...
    $('form#update-account').on('submit', function(event) {                    
