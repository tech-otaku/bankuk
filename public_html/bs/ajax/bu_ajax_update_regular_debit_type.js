...
$(document).ready(function() {
    ...
    $("div#update-regular-debit-type-modal").on("show.bs.modal", function (e) {
        ...
        */ 
        ...
        var DTRowIndex = $('#regular-debit-types').DataTable().row($(e.relatedTarget).parents('tr')).index()   // Based on the total number of rows in a table. With a table containing 5,000 records the row index is between 0 and 4,999.
        $('form#update-regular-debit-type input[type=text][name=dt-row-index]').val(DTRowIndex)
        ...
        $('form#update-regular-debit-type input[type=text][name=dom-row-index]').val(DOMRowIndex)
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Modal Header
                $(e.currentTarget).find('#staticBackdropLabel').html('View | Update Regular Debit Type [Record ID <span class="text-grey">' + recordID + '</span>]')

            // Record ID [Hidden]
                $(e.currentTarget).find('form#update-regular-debit-type input[type=text][name=record-id]').val(data['id'])
            // Entity ID [Read-only]
                $(e.currentTarget).find('form#update-regular-debit-type #type').val(data['type'])
            // Entity Description
                $(e.currentTarget).find('form#update-regular-debit-type #description').val(data['description'])
            // Used By [Info]
                $(e.currentTarget).find('form#update-regular-debit-type span#used-by').text('Used by ' + usedBy + ' ' + recordType + ' records')  
            } else {
            // Update failed
                Swal.fire({
                    title: "Failure",
    ...
    $('form#update-regular-debit-type').on('submit', function(event) {                    
