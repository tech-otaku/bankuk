...
// Populate the update form fields
    ...
    $("div[id^=update-][id$=-modal]").on("show.bs.modal", function (e) {
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Update successful
                ...
                const PopulateFormFields = (async ($w, x, y, z) => {
                    ...
                    $w.find('h5#staticBackdropLabel').html('View | Update ' + tableObject.header + ' [Record ID <span class="text-grey">' + recordID + '</span>]')
                    for (const field of tableObject.fields) {   // Loop through each object in the `fields` array
                        if (field.type === 'datepicker') {
                            ...
                            $w.find('form#' + y + ' #' + field.id).datepicker({             
                             ...
                             $w.find('form#' + y + ' #' + field.id).datepicker("setDate", z[field.column]);
                        }
    ...
    $('form.update-form').on('submit', function(event) {                    // Any form whose class is 'update-form' will be processed by this event handler when it's submitted
