...
$(document).ready(function() {
    ...
    $("div#update-transaction-type-modal").on("show.bs.modal", function (e) {
        ...
        */ 
        ...
        var DTRowIndex = $('#transaction-types').DataTable().row($(e.relatedTarget).parents('tr')).index()   // Based on the total number of rows in a table. With a table containing 5,000 records the row index is between 0 and 4,999.
        $('form#update-transaction-type input[type=text][name=dt-row-index]').val(DTRowIndex)
        ...
        $('form#update-transaction-type input[type=text][name=dom-row-index]').val(DOMRowIndex)
               
        var recordID = $(e.relatedTarget).data("record-id")
        var mysqlTable = $(e.relatedTarget).data("mysql-table")
        var typeUsedBy = $(e.relatedTarget).data("type-used-by")
        var subTypeUsedBy = $(e.relatedTarget).data("subtype-used-by")
        ...
        .done(function( dataReturnedByServer, textStatus, jqXHR) {  // Used instead of the AJAX local callback event 'success: function()'. See https://stackoverflow.com/a/15821199/2518495
            ...
            // Modal Header
                $(e.currentTarget).find('#staticBackdropLabel').html('View | Update Transaction Type [Record ID <span class="text-grey">' + recordID + '</span>]')

            // Record ID [Hidden]
                $(e.currentTarget).find('form#update-transaction-type input[type=text][name=record-id]').val(data['id'])
            // Entity ID [Read-only]
                $(e.currentTarget).find('form#update-transaction-type #type').val(data['type'])
            // Entity Description
                $(e.currentTarget).find('form#update-transaction-type #description').val(data['description'])
            // Used By [Info]
                $(e.currentTarget).find('form#update-transaction-type span#used-by').text('Used by ' + typeUsedBy + ' ' + recordType + ' records as Type and ' + subTypeUsedBy + ' as Sub-Type')  
            } else {
            // Update failed
                Swal.fire({
                    title: "Failure",
    ...
    $('form#update-transaction-type').on('submit', function(event) {                    
